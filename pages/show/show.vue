<template>
	<view class="container">
		...
		<view class="message" v-if="choose==true">
			...
			<view class="s-line"></view>
			<!--有结果-->
			<view class="note" v-if="note.length>0" v-for="(item,index) in note" :key="index">
				<view class="info">
					<view class="avatar">
					...
					<view class="ustime">
						...
						<text class="time">{{timeFormate(item.time)}}</text>
					</view>
				</view>
			...
			<block v-if="note.length>4">
			    <u-loadmore :status="loadingStatus" :load-text="loadText" :margin-top="30" :margin-bottom="30"/>
			</block>
		</view>
		<!--创建留言-->
